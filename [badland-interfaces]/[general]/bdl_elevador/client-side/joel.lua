...
RegisterNUICallback("ButtonClick",function(data,cb)
	...
	elseif data == "heliAEREOH1" then
		DoScreenFadeOut(1000)
		ToggleActionMenu()
		SetTimeout(1400,function()
			SetEntityCoords(ped,-75.63,-825.17,321.3,0,0,0,0)
			SetEntityHeading(ped,255.0)
			TriggerEvent("vrp_sound:source",'elevator-bell',0.5)
			DoScreenFadeIn(1000)
		end)

	elseif data == "heliAEREOT1" then
		DoScreenFadeOut(1000)
		ToggleActionMenu()
		SetTimeout(1400,function()
			SetEntityCoords(ped,-83.48,-836.35,40.56,0,0,0,0)
			SetEntityHeading(ped,255.0)
			TriggerEvent("vrp_sound:source",'elevator-bell',0.5)
			DoScreenFadeIn(1000)
		end)
...
local elevador = {
	...
	{"aereo", -83.48,-836.35,40.56, "[~r~E~w~] Para ~r~USAR~w~ o elevador.", 0,255,0},
	{"aereo", -76.16,-826.29,321.3, "[~r~E~w~] Para ~r~USAR~w~ o elevador.", 0,255,0},
}
-----------------------------------------------------------------------------------------------------------------------------------------
--[ MENU ]-------------------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------------------------
				...
				if distance <= 15 then
					...
					if distance <= 2.3 then
						if IsControlJustPressed(0,38) then
							...
							elseif v[1] == "lspd" then
								SendNUIMessage({ elevLSPD = true })
							elseif v[1] == "aereo" then
								SendNUIMessage({ elevAEREO = true })
							end
						end
					end
				end
