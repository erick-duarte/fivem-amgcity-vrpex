$(function() {
  ...
  $('body').on('submit', '#resultUser', function(e) {
	...
	$.post('http://bdl_emhire/pegarUser', JSON.stringify({ id: valor }));
});
...
$("#contratar1").submit(function(e) {
	e.preventDefault();
	$.post('http://bdl_emhire/contratar', JSON.stringify({
...
$("#demitir1").submit(function(e) {
	e.preventDefault();
	$.post('http://bdl_emhire/demitir', JSON.stringify({
...
$("#promover1").submit(function(e) {
	e.preventDefault();
	$.post('http://bdl_emhire/promover', JSON.stringify({
...
window.addEventListener('message', (event) => {
  ...
  function sendData(name, data) {
	$.post("http://bdl_emhire/" + name, JSON.stringify(data), function(
	  datab
	) {
	  if (datab != "ok") {
		console.log(datab);
