...
RegisterNUICallback("ButtonClick",function(data,cb)
	if data == "vestir-recruta" then
		TriggerServerEvent("recruta")
	
	elseif data == "vestir-soldado" then
		TriggerServerEvent("soldado")

	elseif data == "vestir-cabo" then
		TriggerServerEvent("cabo")

	elseif data == "vestir-sargento" then
		TriggerServerEvent("sargento")

	elseif data == "vestir-tenente" then
		TriggerServerEvent("tenente")

	elseif data == "vestir-capitao" then
		TriggerServerEvent("capitao")
		
	elseif data == "vestir-major" then
		TriggerServerEvent("major")

	elseif data == "vestir-coronel" then
		TriggerServerEvent("coronel")
...
function checkGender()
	local ped = PlayerPedId()
	if vRPex.checkOperation() then
		if GetEntityModel(ped) == GetHashKey("mp_m_freemode_01") then
			SendNUIMessage({ gender = "taticamale" })
		elseif GetEntityModel(ped) == GetHashKey("mp_f_freemode_01") then
			SendNUIMessage({ gender = "taticafemale" })
		end
	else
		if GetEntityModel(ped) == GetHashKey("mp_m_freemode_01") then
