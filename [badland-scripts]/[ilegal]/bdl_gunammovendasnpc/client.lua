...
vSERVER = Tunnel.getInterface("bdl_gunammovendasnpc")
-- VARIAVEIS
local blips = false
local servico = false
local selecionado = 0
...
-- [ ARMAMENTO ] --
local bratvaX = 1402.37
local bratvaY =  1145.06
local bratvaZ = 114.34
...
Citizen.CreateThread(function()
	while true do
		...
		if not servico then
			...
			local distance = Vdist(lostX,lostY,lostZ,x,y,z)
			if distance <= 3 then
				idle = 5
				DrawText3D(lostX,lostY,lostZ,"[~p~E~w~] Para ~p~INICIAR~w~ as entregas.")
...
Citizen.CreateThread(function()
	while true do
		...
		if not servico then
			local ped = PlayerPedId()
			local x,y,z = table.unpack(GetEntityCoords(ped))
			local distance = Vdist(bratvaX,bratvaY,bratvaZ,x,y,z)
			if distance <= 3 then
				idle = 5
				DrawText3D(bratvaX,bratvaY,bratvaZ,"[~p~E~w~] Para ~p~INICIAR~w~ as entregas.")
				if distance <= 1.5 then
					if IsControlJustPressed(0,38) and vSERVER.checkPermission() then
						servico = true
						selecionado = math.random(#locs)
			...
			if servico then
				...
				if distance <= 10 then
					...
					if distance <= 1.2 then
						DrawText3D(locs[selecionado].x,locs[selecionado].y,locs[selecionado].z,"[~r~E~w~] Para ~r~ENTREGAR~w~.")
