...
function src.tryPackProduct()
	...
	if user_id and data and data.inventory then
		...
		if parseInt(quantidade[source]) > 0 then
			local weedpack_amount = parseInt(quantidade[source]/5)
			local timer = 0
			if weedpack_amount >= 1 and weedpack_amount <= 20 then
				timer = 30
			elseif weedpack_amount > 20 and weedpack_amount <= 40 then
				timer = 45
			elseif weedpack_amount > 40 and weedpack_amount <= 60 then
				timer = 60
			elseif weedpack_amount > 60 and weedpack_amount <= 80 then
				timer = 75
			elseif weedpack_amount > 80 and weedpack_amount <= 100 then
				timer = 90
			elseif weedpack_amount > 100 then
				timer = 120
			end

			if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("maconha-embalada")*parseInt(weedpack_amount) <= vRP.getInventoryMaxWeight(user_id) then
				vRP.antiflood(source,"Fabricação Maconha",3)
				vRP.tryGetInventoryItem(user_id,"maconha",weedpack_amount*5)
				TriggerClientEvent("progress",source,timer*1000)
				TriggerClientEvent("cancelando",source,true)
				vCLIENT.freezPed(source, timer)
				vRPclient._playAnim(source,true,{{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"}},true)
				SetTimeout(timer*1000,function()
					TriggerClientEvent("cancelando",source,false)
					vRP.giveInventoryItem(user_id,"maconha-embalada",weedpack_amount)
					vRPclient._stopAnim(source,false)
				end)
				return true
			end
...
function src.tryPackCoca()
	...
	quantidade[source] = 0
	if user_id and data and data.inventory then
		vRP.antiflood(source,"Fabricação Cocaina",3)
		for i,o in pairs(data.inventory) do
			if i == "cocaina" then
				quantidade[source] = o.amount
			end
		...
		if parseInt(quantidade[source]) > 0 then
			local cokepack_amount = parseInt(quantidade[source])
			local timer = 0
			if cokepack_amount >= 1 and cokepack_amount <= 20 then
				timer = 30
			elseif cokepack_amount > 20 and cokepack_amount <= 40 then
				timer = 45
			elseif cokepack_amount > 40 and cokepack_amount <= 60 then
				timer = 60
			elseif cokepack_amount > 60 and cokepack_amount <= 80 then
				timer = 75
			elseif cokepack_amount > 80 and cokepack_amount <= 100 then
				timer = 90
			elseif cokepack_amount > 100 then
				timer = 120
			end

			if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("coca-embalada")*parseInt(cokepack_amount) <= vRP.getInventoryMaxWeight(user_id) then
				vRP.tryGetInventoryItem(user_id,"cocaina",cokepack_amount)
				TriggerClientEvent("progress",source,timer*1000)
				TriggerClientEvent("cancelando",source,true)
				vCLIENT.freezPed(source, timer)
				vRPclient._playAnim(source,true,{{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"}},true)
				SetTimeout(timer*1000,function()
					TriggerClientEvent("cancelando",source,false)
					vRP.giveInventoryItem(user_id,"coca-embalada",cokepack_amount)
					vRPclient._stopAnim(source,false)
				end)
				return true
			end
...
--	if user_id then
--		if vRP.hasPermission(user_id,"verdes.permission") then
--			local tempo = 0
...
--	if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("metasuja")*parseInt(amount) <= vRP.getInventoryMaxWeight(user_id) then
	if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("metasuja")*5 <= vRP.getInventoryMaxWeight(user_id) then
		TriggerClientEvent("cancelando",source,true)
		vRPclient._playAnim(source,true,{{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"}},true)
		TriggerClientEvent("progress",source,2000)
		SetTimeout(2000,function()
			TriggerClientEvent("cancelando",source,false)
			vRPclient._stopAnim(source,false)
--			vRP.giveInventoryItem(user_id,"metasuja",parseInt(amount))
			vRP.giveInventoryItem(user_id,"metasuja",5)
		end)
		return true
	else
		TriggerClientEvent("Notify",source,"negado","Peso insuficiente.</b>")
...
--	if vRP.hasPermission(user_id,"brancos.permission") then
...
function src.tryWashItem()
	...
	local user_id = vRP.getUserId(source)
	local identity = vRP.getUserIdentity(user_id)
	wash_amount[source] = vRP.prompt(source,"Coloque a quantidade a ser lavada:","")
	if vRP.hasPermission(user_id,"brancos.permission") then
		if parseInt(wash_amount[source]) >= 5 then
			if vRP.tryGetInventoryItem(user_id,"metasuja",parseInt(wash_amount[source])) then
--				vRP.giveInventoryItem(user_id,"metasuja",wash_amount[source])
				return true
			else
				TriggerClientEvent("Notify",source,"negado","Valor inválido.")
			end
		else
			TriggerClientEvent("Notify",source,"negado","A quantidade mínima é de <b>5x Metanfetamina Suja</b>")
		end
	end
end

function src.tryGiveMethClear()
	local source = source
	local user_id = vRP.getUserId(source)
--	if vRP.tryGetInventoryItem(user_id,"metasuja",parseInt(wash_amount[source])) then
		vRP.giveInventoryItem(user_id,"metanfetamina",wash_amount[source]/5)
		return true
--	end
end
...
function src.tryPackMeth()
	...
	quantidade[source] = 0
	if user_id and data and data.inventory then
		vRP.antiflood(source,"Fabricação Metanfetamina",3)
		for i,o in pairs(data.inventory) do
			if i == "metanfetamina" then
				quantidade[source] = o.amount
			end
		end
		if parseInt(quantidade[source]) > 0 then
			local methpack_amount = parseInt(quantidade[source])
			local timer = 0
			...
			if vRP.getInventoryWeight(user_id)+vRP.getItemWeight("meta-embalada")*parseInt(methpack_amount) <= vRP.getInventoryMaxWeight(user_id) then
				vRP.tryGetInventoryItem(user_id,"metanfetamina",methpack_amount)
				TriggerClientEvent("progress",source,timer*1000)
				TriggerClientEvent("cancelando",source,true)
				vCLIENT.freezPed(source, timer)
				vRPclient._playAnim(source,true,{{"amb@prop_human_parking_meter@female@idle_a","idle_a_female"}},true)
				SetTimeout(timer*1000,function()
					TriggerClientEvent("cancelando",source,false)
					vRP.giveInventoryItem(user_id,"meta-embalada",methpack_amount)
					vRPclient._stopAnim(source,false)
				end)
				return true
			end
