...
local cocaPZ = 218.65
local embalarCocaX = 1911.83
local embalarCocaY = 6455.1
local embalarCocaZ = 83.84

local embalarCocaX2 = 1913.69
local embalarCocaY2 = 6454.68
local embalarCocaZ2 = 83.84
-- -1107.1, 4942.38, 218.95
-- 1913.69,6454.68,83.84

-- 1911.83,6455.1,83.84

-- PASSOS DO FARM DE COCA
local stepCocaX = -1108.75
local stepCocaY = 4952.17
local stepCocaZ = 218.65
-- -1108.75,4952.17,218.65
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(weedchair) do
			...
			if distance <= 2.3 and not weedchairprogress then
				idle = 5
...
Citizen.CreateThread(function()
	while true do
		...
		local distance5 = Vdist(embalarCocaX2,embalarCocaY2,embalarCocaZ2,x,y,z)
		...
		if distance4 <= 2.3 or distance5 <= 2.3 and not embalando then
			idle = 1
			DrawText3Ds(embalarCocaX,embalarCocaY,embalarCocaZ, "[~g~E~w~] Para ~g~EMBALAR~w~ a droga.")
			DrawText3Ds(embalarCocaX2,embalarCocaY2,embalarCocaZ2, "[~g~E~w~] Para ~g~EMBALAR~w~ a droga.")
...
Citizen.CreateThread(function()
	while true do
		...
		if distance3 < 3 and metastep == 2 then
			if parseInt(cl_progress) >= 100 then
				DrawText3Ds(1490.43,6391.15,20.79, "[~y~E~w~] Para ~y~PEGAR~w~ a metanfetamina suja.")
			elseif parseInt(cl_progress) > 0 and parseInt(cl_progress) < 100 then
...
Citizen.CreateThread(function()
	while true do
		...
		local distance2 = Vdist(-151.71,2123.09,170.15,x,y,z)
		if general_distance <= 30 then
			idle = 5
...
--					DrawText3Ds(1494.64,6395.5,20.79, "DROGA LAVADA...")
			...
			if distance <= 8 and isready then
				if washstep == 1 then
					DrawText3Ds(1494.64,6395.5,20.79, "[~y~E~w~] Para ~y~PEGA~w~ a metanfetamina limpa.")
				end
				if distance <= 1.2 then
					if IsControlJustPressed(0,38) then
						if vSERVER.tryGiveMethClear() then
							washstep = 0
							lavada = false
							incl_process = false
							inwashprocess = false
		...
		if distance2 <= 2.3 and not facembalando then
			idle = 5
			DrawText3Ds(-151.71,2123.09,170.15, "[~y~E~w~] Para ~y~EMBALAR~w~ a metanfetamina.")
			if distance2 <= 1.2 then
				if IsControlJustPressed(0,38) then
					facembalando = true
					vSERVER.tryPackMeth()
            ...
            if not onMenu then
                ...
                if distance2 <= 2 then
                    ...
                    if distance2 <= 1.1 then
                        if IsControlJustPressed(0,38) and vSERVER.getUserPermission("brancos.permission") then
                            ToggleActionMenu()
                        end
                    end
                end
