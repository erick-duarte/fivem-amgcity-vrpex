...
Citizen.CreateThread(function()
	while true do
		...
		if not servico then
			...
			local distance = Vdist(vermelhosX,vermelhosY,vermelhosZ,x,y,z)
			if distance <= 3 then
				idle = 5
				DrawMarker(23,vermelhosX,vermelhosY,vermelhosZ-1,0,0,0,0,180.0,130.0,1.2,1.2,1.2,130,0,255,50,0,0,0,1)
				if distance <= 1.5 then
					DrawText3D(vermelhosX,vermelhosY,vermelhosZ,"[~p~E~w~] Para ~p~INICIAR~w~ as entregas.")
...
Citizen.CreateThread(function()
	while true do
		...
		if not servico then
			local ped = PlayerPedId()
			local x,y,z = table.unpack(GetEntityCoords(ped))
			local distance = Vdist(verdesX,verdesY,verdesZ,x,y,z)
			if distance <= 3 then
				idle = 5
				DrawMarker(23,verdesX,verdesY,verdesZ-1,0,0,0,0,180.0,130.0,1.2,1.2,1.2,25,255,25,50,0,0,0,1)
				if distance <= 1.5 then
					DrawText3D(verdesX,verdesY,verdesZ,"[~g~E~w~] Para ~g~INICIAR~w~ as entregas.")
					if IsControlJustPressed(0,38) and vSERVER.checkPermission() then
						servico = true
						selecionado = math.random(#locs)
						CriandoBlip(locs,selecionado)
...
Citizen.CreateThread(function()
	while true do
		...
		if not servico then
			local ped = PlayerPedId()
			local x,y,z = table.unpack(GetEntityCoords(ped))
			local distance = Vdist(brancosX,brancosY,brancosZ,x,y,z)
			if distance <= 3 then
				idle = 5
				DrawMarker(23,brancosX,brancosY,brancosZ-1,0,0,0,0,180.0,130.0,1.2,1.2,1.2,255,255,25,50,0,0,0,1)
				if distance <= 1.5 then
					DrawText3D(brancosX,brancosY,brancosZ,"[~y~E~w~] Para ~y~INICIAR~w~ as entregas.")
					if IsControlJustPressed(0,38) and vSERVER.checkPermission() then
						servico = true
						selecionado = math.random(#locs)
						CriandoBlip(locs,selecionado)
