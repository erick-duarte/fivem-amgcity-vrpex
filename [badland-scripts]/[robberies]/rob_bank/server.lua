...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookbankrobbery = "https://discord.com/api/webhooks/845401582198587412/FbbxHMQxELXSeQs4hA6JXPmk_73-MclHiyc4AAoetGQtqVsCfxyrdoxmRUpq2HKcFJ_0"
...
-- CHECKPOLICE
-----------------------------------------------------------------------------------------------------------------------------------------
function func.CheckPolice(bank)
    local source = source
    local user_id = vRP.getUserId(source)
    if user_id then
        if not vRP.hasPermission(user_id,"lspd.permission") or vRP.hasPermission(user_id,"ems.permission") or vRP.hasPermission(user_id,"bennys.permission") then
            ...
            elseif (os.time()-timedown) <= 1800 then
                ...
                return false
            elseif bank == "Fleeca Bank Highway" or bank == "Fleeca Bank Top" or bank == "Fleeca Bank Invader" and #policia >= 4 then
                return true
            elseif bank == "Fleeca Bank Paleto" and #policia >= 6 then
                return true
            elseif bank == "Central Bank" and #policia >= 8 then
                return true
            end
        else
            return false
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1000)
        if os.date("%H:%M:%S") == "20:30:00" or os.date("%H:%M:%S") == "02:00:00" or os.date("%H:%M:%S") == "16:30:00" or os.date("%H:%M:%S") == "21:00:00" then
            selecionado = math.random(16)
            print(selecionado)
            TriggerEvent("DropSystem:create","blankcard",1,spawns[selecionado].x,spawns[selecionado].y,spawns[selecionado].z,3600)
        end
    end
end)