...
emPLen = Tunnel.getInterface("lenhador_coletar")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterNetEvent('lenhador_coletar:permitircoleta')
AddEventHandler('lenhador_coletar:permitircoleta',function()
	permitido = true
end)

RegisterNetEvent('lenhador_coletar:cancelarcoleta')
AddEventHandler('lenhador_coletar:cancelarcoleta',function()
	permitido = false
end)
			...
			if not processo then
				for _,func in pairs(arvores) do
					...
					if distancia <= 50 and list[i] == nil then
						tempo = 1
						DrawMarker(21,x,y,z,0,0,0,0.0,0,0,0.5,0.5,0.4,255,0,0,50,0,0,0,1)
						if distancia <= 1.2 then
							...
							if IsControlJustPressed(0,38) then
								if GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_HATCHET") or GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_BATTLEAXE") then
									...
									TriggerEvent("Notify","negado","Você precisa de um <b>Machado</b> ou <b>Machado de Batalha</b>.")
								end
							end
						end
					end
