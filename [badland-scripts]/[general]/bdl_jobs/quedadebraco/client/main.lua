...
function timer()
    ...
    while GetGameTimer() - T < 1000 do
      Wait(0)
      Draw2DText(0.4, 0.4, ("~s~VAI ~w~!"), 3.0)
    end
end
...
AddEventHandler('evy_arm:start_cl', function()
  ...
  if place == 1 then
    ...
    if grade >= 0.90 then
      ...
      TriggerEvent("Notify","sucesso","Você ganhou !",5000)
    elseif grade <= 0.10 then
      PlayAnim(PlayerPedId(), "mini@arm_wrestling", "win_a_ped_b", 2)
    TriggerEvent("Notify","negado","Você perdeu !",5000)
    end
    Wait(4000)
  ...
  elseif place == 2 then
    ...
    if grade <= 0.10 then
      PlayAnim(PlayerPedId(), "mini@arm_wrestling", "win_a_ped_a", 2)
      TriggerEvent("Notify","sucesso","Você ganhou !")
    elseif grade <= 0.10 then
      PlayAnim(PlayerPedId(), "mini@arm_wrestling", "win_a_ped_b", 2)
    TriggerEvent("Notify","negado","Você perdeu !")
    end
    Wait(4000)
    TriggerServerEvent('evy_arm:disband_sv', GetEntityCoords(PlayerPedId()))
    return
...
AddEventHandler('evy_arm:check_cl', function(args)
  ...
  if args == 'place1' then
    ...
    while not started do
      ...
      if IsControlPressed(2, 167) or IsPedRagdoll(PlayerPedId()) then--or IsControlPressed(2, 200) or IsControlPressed(2, 214) then
  ...
  elseif args == 'noplace' then
    TriggerEvent("Notify","negado","Todas as mesas estão ocupadas.")
  end
end)
