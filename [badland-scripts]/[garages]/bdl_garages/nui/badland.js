		...
		$('#garagem').html(`
			${nameList.map((item) => (`
					...
					<div class="item-imageName">
						<img src="http://177.159.205.245:63248/dealership/${item.name}.png">
					</div>
					...
					<div class="item-text" style="border: 0;">
                        <div class="item-taxa">IPVA: ${item.ipva}</div>
                    </div>
				</div>
			`)).join('')}
		`);
...
const updateGaragesAereo = () => {
    $.post('http://bdl_garages/myVehiclesAereo',JSON.stringify({}),(data) => {
		const nameList = data.vehicles.sort((a,b) => (a.name2 > b.name2) ? 1: -1);
		$('#garagem').html(`
			${nameList.map((item) => (`
					<div onclick="selectItem(this)" class="item" data-item-name="${item.name}">
