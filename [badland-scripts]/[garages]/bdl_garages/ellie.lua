...
vRP._prepare("creative/set_update_vehicles","UPDATE vrp_user_vehicles SET engine = @engine, body = @body, fuel = @fuel, porta0 = @porta0, porta1 = @porta1, porta2 = @porta2, porta3 = @porta3, porta4 = @porta4, porta5 = @porta5 WHERE user_id = @user_id AND vehicle = @vehicle")
...
vRP._prepare("creative/set_desmanchado","UPDATE vrp_user_vehicles SET desmanchado = 1 WHERE user_id = @user_id AND vehicle = @vehicle")
...
-- [ WEBHOOKS ] --
local webhookvehs = "https://discord.com/api/webhooks/845400803912056843/WhoN8i1EL5iAvcJDApJ7R06VW_uTIYxBgL45A4WVDEXWONW8yTEVMVsafe12HgxTCZXd"
local webhookdesmanche = "https://discord.com/api/webhooks/845400919033118730/BcDRBg8QAFqojKUqM4xKIAP-8o5wneRmubjZ8FqYEgsPM74i_rnPKCzqRfdgWgP-hsrw"
...
local garages = {
	...
	-- [ LSPD ] --
	[14] = { ['name'] = "PRESIDIO", ['payment'] = false, ['perm'] = "lspd.permission" },
	[16] = { ['name'] = "POLICIA", ['payment'] = false, ['perm'] = "policiaamg.permission" },
	[17] = { ['name'] = "TATICA", ['payment'] = false, ['perm'] = "tatica.permission" },
	[18] = { ['name'] = "POLICIAH", ['payment'] = false, ['perm'] = "lspd.permission" },
	-- [ EMS ] --
...
--	[621] = { ['name'] = "Kart", ['payment'] = false, ['public'] = true },
	[622] = { ['name'] = "Pescador", ['payment'] = false, ['public'] = true },
	[623] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
--AEREO
	[700] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
	[701] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
	[702] = { ['name'] = "Garagem", ['payment'] = false, ['public'] = true },
--RODOVIARIA
	[703] = { ['name'] = "Bike", ['payment'] = false, ['public'] = true },
--PRISÃO
	[704] = { ['name'] = "Bike", ['payment'] = false, ['public'] = true },
}
-- [ WORK VEHICLES ARRAY ] --
local workgarage = {
	["POLICIA"] = {
...
function src.myVehicles(work)
	...
	if user_id then
		if workgarage[work] then
			...
			return myvehicles
		elseif work == "Aereo" then
			local vehicle = vRP.query("creative/get_vehicle",{ user_id = parseInt(user_id) })
			for k,v in pairs(vehicle) do

				if vRP.vehicleType(tostring(vehicle[k].vehicle)) == "aereo" then

					if parseInt(os.time()) <= parseInt(vehicle[k].time+24*60*60) then
						status = "<span class=\"red\">$"..vRP.format(parseInt(vRP.vehiclePrice(vehicle[k].vehicle)*0.02)).."</span>"
					elseif vehicle[k].detido == 1 then
						status = "<span class=\"orange\">$"..vRP.format(parseInt(vRP.vehiclePrice(vehicle[k].vehicle)*0.02)).."</span>"
					elseif vehicle[k].desmanchado == 1 then
					...
					if vehicle[k].detido == 1 or vehicle[k].desmanchado == 1 then
				...
				for k,v in pairs(address) do
					if v.home == work then
						for k2,v2 in pairs(vehicle) do
							if vRP.vehicleType(tostring(vehicle[k2].vehicle)) ~= "aereo" then
...
--									status = "<span class=\"orange\">$"..vRP.format(parseInt(vRP.vehiclePrice(vehicle[k2].vehicle)*0.02)).."</span>"
--								else
--									status = "<span class=\"green\">Gratuita</span>"
--								end
								if parseInt(os.time()) <= parseInt(vehicle[k2].time+24*60*60) then
									status = "<span class=\"red\">$"..vRP.format(parseInt(vRP.vehiclePrice(vehicle[k2].vehicle)*0.02)).."</span>"
								elseif vehicle[k2].detido == 1 then
									status = "<span class=\"orange\">$"..vRP.format(parseInt(vRP.vehiclePrice(vehicle[k2].vehicle)*0.02)).."</span>"
								elseif vehicle[k2].desmanchado == 1 then
...
--									vehstate = "<span class=\"red\"><i class=\"fas fa-car\"></i></span>"
--								end
								if vehicle[k2].detido == 1 or vehicle[k2].desmanchado == 1 then
									vehstate = "<span class=\"red\"><i class=\"fas fa-car\"></i></span>"
								end
...
AddEventHandler("desmancheVehicles",function()
	...
	if user_id then
		...
		if vehicle and placa then
			...
			if puser_id then
				vRP.execute("creative/set_desmanchado",{ user_id = parseInt(puser_id), vehicle = vname, desmanchado = 1, time = parseInt(os.time()) })
	...
	if vRP.hasPermission(user_id,"staff.permission") then
		if tonumber(args[1]) then
			...
			if vehicle then
				...
				TriggerClientEvent('Notify',source, 'negado', 'Comando dado de forma incorreta, use a estrutura /dv ou /dv [raio]',5000)
			end
		end
	end
end)
...
-- [ TRY DELETE ] --
function src.tryDelete(vehid,vehengine,vehbody,vehfuel,porta0,porta1,porta2,porta3,porta4,porta5)
	if vehlist[vehid] and vehid ~= 0 then
		...
		local vheng = tostring(vehengine) --Corrigindo bug do fivem
		if vheng == "-nan" then
			vehengine = 0
		end
		...
		if vehicle[1] ~= nil then
			vRP.execute("creative/set_update_vehicles",{ user_id = parseInt(user_id), vehicle = vehname, engine = parseInt(vehengine), body = parseInt(vehbody), fuel = parseInt(vehfuel), porta0 = parseInt(porta0), porta1 = parseInt(porta1), porta2 = parseInt(porta2), porta3 = parseInt(porta3), porta4 = parseInt(porta4), porta5 = parseInt(porta5) })
...
function src.tryDeleteLimbo(vehid)
	if vehlist[vehid] and vehid ~= 0 then
		local user_id = vehlist[vehid][1]
		local vehname = vehlist[vehid][2]
		local player = vRP.getUserSource(user_id)
						...
						if parseInt(v.garage) == 1 then
							...
							if resultOwner[1] then
								if parseInt(os.time()) >= parseInt(resultOwner[1].tax+24*15*60*60) then
									TriggerClientEvent("Notify",source,"aviso","O IPTU da residência está atrasado.",10000)
									return false
								else
...
--RegisterCommand('destravar',function(source,args,rawCommand)
--	local source = source
--	local user_id = vRP.getUserId(source)
--	local identity = vRP.getUserIdentity(user_id)
--	if user_id then
--		if vRPclient.isInVehicle(source) then
--			local vehicle,vnetid,placa,vname,lock,banned = vRPclient.vehList(source,7)
--			if vehicle and identity.registration == placa then
--				if vRP.request(source,"Você deseja pagar $3.000 dólares para remover seu veículo do patio?",30) then
--					if vRP.tryFullPayment(user_id,parseInt(3000)) then
--						TriggerClientEvent("progress",source,5000,"travar/destravar")
--						SetTimeout(5000,function()
--							vCLIENT.vehicleAnchor(source,vehicle)
--						end)
	...
	if user_id then
		if args[1] and parseInt(args[2]) > 0 then
			...
			if myvehicles[1] then
				if vRP.vehicleType(tostring(args[1])) == "exclusive" or vRP.vehicleType(tostring(args[1])) == "rental" or vRP.vehicleType(tostring(args[1])) == "vips" and not vRP.hasPermission(user_id,"staff.permission") then
...
AddEventHandler("Vehiclesdesmanche",function()
	...
	if user_id then
		...
		if vehicle and placa then
			local puser_id = vRP.getUserByRegistration(placa)
			if puser_id then
				vRP.execute("creative/set_desmanchado",{ user_id = parseInt(puser_id), vehicle = vname })
				vCLIENT.deleteVehicle(source,vehicle)
				local identity = vRP.getUserIdentity(user_id)
