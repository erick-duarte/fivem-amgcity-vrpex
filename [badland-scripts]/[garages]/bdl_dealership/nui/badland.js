...
const updateCarros = () => {
	...
	$('.testedrivee').css("opacity","1");
	$.post("http://bdl_dealership/requestCarros",JSON.stringify({}),(data) => {
		let i = 0;
		const nameList = data.veiculos.sort((a,b) => (a.price > b.price) ? 1: -1);
		$('#inicio').html(`
			${nameList.map((item) => (`
				<div class="model" data-name-key="${item.k}">
					<div class="itemImage">
						<img src="http://177.159.205.245:63248/dealership/${item.k}.png">
					</div>
					
					<div class="name"><p>${item.nome}</p></div>
					<div class="valor">$${formatarNumero(item.price)},00</div>
					<div class="malas">Mala: ${item.chest}Kg</div>
...
const updateOffRoad = () => {
	$('.comprar').css("opacity","1");
	$('.testedrivee').css("opacity","1");
	$.post("http://bdl_dealership/requestOffRoad",JSON.stringify({}),(data) => {
		let i = 0;
		const nameList = data.veiculos.sort((a,b) => (a.price > b.price) ? 1: -1);
		$('#inicio').html(`
			${nameList.map((item) => (`
...
const updateVans = () => {
	$('.comprar').css("opacity","1");
	$('.testedrivee').css("opacity","1");
	$.post("http://bdl_dealership/requestVans",JSON.stringify({}),(data) => {
		let i = 0;
		const nameList = data.veiculos.sort((a,b) => (a.price > b.price) ? 1: -1);
		$('#inicio').html(`
			${nameList.map((item) => (`
...
//	$('.testedrivee').css("opacity","1");
	$.post("http://bdl_dealership/requestVips",JSON.stringify({}),(data) => {
		let i = 0;
...
$('.testedrivee').click(function(){
	let $el = $('.model');
	let isActive = $el.hasClass('active');
	if (isActive == true) {
		$('.confirmScreenTest').fadeIn(250);
	} else {
		return
	}
});
...
$('.voltartestDrive').click(function(){
	$('.confirmScreenTest').fadeOut(250);
});
...
$(document).on("click",".confirmartestDrive",function(){
	let $el = $('.model.active');
	if($el){
		$.post("http://bdl_dealership/testDrive",JSON.stringify({
			name: $el.attr('data-name-key')
		}));
		$('.confirmScreenTest').fadeOut(250);
	}
