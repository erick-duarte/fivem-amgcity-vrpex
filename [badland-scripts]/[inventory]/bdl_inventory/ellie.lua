...
local dropLog = "https://discord.com/api/webhooks/845402214205751328/FGnU2FSfI8NuKTjywzxpbxlPfx-7kp8E-rogljzqm2XrCu5rUmHLwFnjDqVxPfF-E0Zz"
local sendLog = "https://discord.com/api/webhooks/845402313762013184/lpW7hh6j9P_xkFMBYuwp7RFNFxb9xb9obSjhnSwaIx2I1CXj0uheG9YhXgzw02RGkQ6J"
local equipLog = "https://discord.com/api/webhooks/845402422751133766/dUH9dTIxAqV-DrQQ0zVB-XyX-P_azp8iRJc4VJaVepeZlpjuIqAdGr0MGleOnQ8WN01h"
local ammoLog = "https://discord.com/api/webhooks/845402542297186334/Q6bZ9dJg6D1F1YU6728FSkqVrRwPNmWBWXazoHgf2yiSXupJBWTLEU2uAAw_mJtbZcHX"
...
function vRPex.useItem(itemName,type,ramount)
	...
	if user_id and ramount ~= nil and parseInt(ramount) >= 0 and not actived[user_id] and actived[user_id] == nil then
		if type == "usar" then
			...
			elseif itemName == "garrafavazia" then
				local src = source
				actived[user_id] = true
				TriggerClientEvent('Creative:Update',source,'updateMochila')
--				vCLIENT.watercooler(source)
				TriggerClientEvent('watercooler',source)
				vCLIENT.closeInventory(source)
				TriggerClientEvent('cancelando',source,true)
			...
			elseif itemName == "componenteeletronico" then
				local src = source
				actived[user_id] = true
				TriggerClientEvent('Creative:Update',source,'updateMochila')
				TriggerClientEvent("progress",source,12000,"tomando")
				TriggerClientEvent("AMG_Lavagem:resetvida",source)
				vCLIENT.closeInventory(source)
				TriggerClientEvent('cancelando',source,true)
				SetTimeout(6000,function()
					actived[user_id] = nil
			...
			elseif itemName == "placa" then
				if not vRPclient.isInVehicle(source) then
					...
					if vehicle then
						...
						vCLIENT.closeInventory(source)
						vRPclient._playAnim(source,false,{{"mini@repair","fixing_a_player"}},true)
						TriggerClientEvent("progress",source,5000,"reparando veículo")
						SetTimeout(5000,function()
							actived[user_id] = nil
							TriggerClientEvent('cancelando',source,false)
--							TriggerEvent("setPlateEveryone","TESTE123")
							TriggerClientEvent("cloneplates",source,vehicle)
							vRPclient._stopAnim(source,false)
							TriggerClientEvent("Notify",source,"sucesso","Placa clonada com sucesso.",8000)
						end)
					end
				else
					TriggerClientEvent("Notify",source,"negado","Você precisa está fora do veiculos.",8000)
				end
			...
			elseif itemName == "leite" then
				...
				if vRP.tryGetInventoryItem(user_id,"leite",1) then
					...
					actived[user_id] = true
					TriggerClientEvent('Creative:Update',source,'updateMochila')
					vRPclient._CarregarObjeto(src,"amb@world_human_drinking@beer@male@idle_a","idle_a","prop_cs_milk_01",49,28422)
					vCLIENT.closeInventory(source)
					TriggerClientEvent("progress",source,10000,"tomando")
			...
			elseif itemName == "barracho" then
				...
				if vRP.tryGetInventoryItem(user_id,"barracho",1) then
					actived[user_id] = true
					TriggerClientEvent('Creative:Update',source,'updateMochila')
					vRPclient._CarregarObjeto(src,"amb@world_human_drinking@beer@male@idle_a","idle_a","prop_beer_bar",49,28422)
					vCLIENT.closeInventory(source)
					TriggerClientEvent("progress",source,10000,"tomando")
			...
			elseif itemName == "patriot" then
				...
				if vRP.tryGetInventoryItem(user_id,"patriot",1) then
					actived[user_id] = true
					TriggerClientEvent('Creative:Update',source,'updateMochila')
					vRPclient._CarregarObjeto(src,"amb@world_human_drinking@beer@male@idle_a","idle_a","prop_beer_patriot",49,28422)
					vCLIENT.closeInventory(source)
					TriggerClientEvent("progress",source,10000,"tomando")
