...
--function RPex.watercooler()
RegisterNetEvent('watercooler')
AddEventHandler('watercooler',function()
	local x,y,z = table.unpack(GetEntityCoords(PlayerPedId()))
    local cooler = GetClosestObjectOfType(x,y,z,1.5,-1369928609,0,0,0)
    SetEntityAsMissionEntity(cooler, true, true)
    if DoesEntityExist(cooler) then
        distanceCooler = GetEntityCoords(cooler, 0)
        local distance = GetDistanceBetweenCoords(distanceCooler['x'], distanceCooler['y'], distanceCooler['z'], x,y,z, true)
        if distance <= 1.5 and vSERVER.searchCooler() then
			TriggerEvent("progress",12000,"enchendo garrafa")
			SetTimeout(6000,function()
				vSERVER.coolerPayment()
			end)
        end
    end
end)
...
--           if vSERVER.searchCooler(coolerCoords["x"]) then
--               TriggerEvent("progress",12000,"enchendo garrafa")
--               SetTimeout(6000,function()
--                   vSERVER.coolerPayment()
--               end)
