...
--                    local pjob = SVIDclient.getJob(GetPlayerServerId(id))
--                    players[id] = {pid,pjob}
                    players[id] = pid
	        ...
	        for _, id in ipairs(GetActivePlayers()) do
		    	...
		    	if admim and (PlayerPedId() ~= GetPlayerPed( id ) or mostraSeuID)then
			    	if ((distance < distancia)) then
			    		DrawText3D(x2, y2, z2+2, players[id], 255, 255, 255)
--                        DrawText3D(x2, y2, z2+1, players[id][2], 255, 255, 255)
			    	end
			    end
			end
		end
