...
--            local encode = json.encode (v)
--            print (encode)
            if v.ocupado and v.spawnado == 0 then
                vRP.execute('vRP/set_spawnado', {user_id = v.dono, vehicle = v.modelo})
                vRPvd.spawnarCarro(user_id, v)
            elseif v.spawnado == 1 and v.dono == user_id then
                vRPvd.spawnarCarro(user_id, v)
            end
        end
    end
...
function vRPvd.restartScript()
	...
	local user_id = vRP.getUserId(source)
    local player = vRP.getUserSource(user_id)
    VDclient.setVendas22(player, vendas)
    if first_spawn then
        first_spawn = false
        for k,v in pairs(vendas) do
            if v.ocupado and v.spawnado == 0 then
...
function vRPvd.loglog(log)
    local encode = json.encode (log)
    print (encode)
end
