...
-- [ WEBHOOK LINKS ] --
local logmultas = "https://discord.com/api/webhooks/843340678083641357/jh_y0W2HWirD_nynVipE-qQtTctF-oSyV_0xla5sNeEt82vx_8fs3py6_X19qNWH1U9t"
local logocorrencias = "https://discord.com/api/webhooks/843340713163882537/EyTytN3MQgQlECHlKm5jDH1ERmDddcdWfKu0SvqJgVfhbSLGLFEUtSKdUM5xKkypnt15"
local logdetido = "https://discord.com/api/webhooks/843340766418829322/jrc4yqjqbQak-9VSYkytX4siFAqlhuMcMGALGWIjR2PkJPWyVriMTISQDqcLUoUbphqM"
local logprender = "https://discord.com/api/webhooks/843340800431095849/nQ776nfUH0RC0muvCaojS0BMnU4tT3PbnYINxXtzXEULXc3aRQopSUC0Q_ViG54Pz2hL"
...
RegisterCommand('ptr', function(source,args,rawCommand)
 	...
 	if vRP.hasPermission(user_id,"lspd.permission") or vRP.hasPermission(user_id,"staff.permission") then
		...
		TriggerClientEvent("Notify",source,"importante", "Atualmente <b>"..#oficiais.." Oficiais</b> em serviço.")
	end
end)
        ...
        if vehicle then
            ...
            if rows[1] then
                if parseInt(rows[1].detido) == 1 then
					...
					vRPclient.playSound(source,"Hack_Success","DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS")
					TriggerClientEvent('deletarveiculo',source,vnetid)
                end
            end
        end
    end
...
--			randmoney = math.random(90,150)
--			vRP.giveMoney(user_id,parseInt(randmoney))
			TriggerClientEvent("Notify",source,"sucesso","Prisão efetuada com sucesso.",5000)
--			TriggerClientEvent("Notify",source,"importante","Você recebeu <b>$"..vRP.format(parseInt(randmoney)).." dólares</b> de bonificação.")
			TriggerClientEvent("Notify",nplayer,"importante","Você foi preso por <b>"..vRP.format(parseInt(args[2])).." meses</b>.<br><b>Motivo:</b> "..crimes..".",5000)
			vRPclient.playSound(source,"Hack_Success","DLC_HEIST_BIOLAB_PREP_HACKING_SOUNDS")
		end 
	end
end)
	...
	if vRP.hasPermission(user_id,"staff.permission") or vRP.hasPermission(user_id,"lspd.permission") then
		if args[1] then
			...
			if nuser_id then
				...
				TriggerClientEvent('chatMessage',source,"------------------------------ RG ------------------------------")
				TriggerClientEvent('chatMessage',source,"Nome: ",{65,130,255},""..identity.name.." "..identity.firstname.."")
				TriggerClientEvent('chatMessage',source,"CPF: ",{65,130,255},""..vRP.format(identity.user_id).."")
				TriggerClientEvent('chatMessage',source,"Identidade: ",{65,130,255},""..identity.registration.."")
				TriggerClientEvent('chatMessage',source,"Multas: ",{65,130,255},""..vRP.format(parseInt(valormultas)).."")
...
RegisterCommand('id',function(source,rawCommand)	
	...
	if nuser_id then
		local identity = vRP.getUserIdentity(nuser_id)
		TriggerClientEvent('chatMessage',source,"---------- REGISTRO ----------")
		TriggerClientEvent('chatMessage',source,"Identidade: ",{255,0,0},""..identity.registration.."")
		TriggerClientEvent('chatMessage',source,"------------------------------")
	...
	if player then
		SetTimeout(60000,function()
			...
			if parseInt(tempo) >= 1 then
				TriggerClientEvent("Notify",player,"importante","Ainda vai passar <b>"..parseInt(tempo).." meses</b> preso.",5000)
			...
			elseif parseInt(tempo) == 0 then
				...
				TriggerClientEvent("Notify",player,"importante","Sua sentença terminou, esperamos não ve-lo novamente.",5000)
			end
