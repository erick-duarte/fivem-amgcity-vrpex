...
$(document).ready(function(){
	window.addEventListener("message",function(event){
		...
		if (event["data"]["talking"] == true){
			$("#voice").css("background","#333 url(https://cdn.discordapp.com/attachments/839531718889439275/846765021866098688/micOn.png)");
		} else {
			$("#voice").css("background","#222 url(https://cdn.discordapp.com/attachments/839531718889439275/846765020523135056/micOff.png)");
			...
			if (event["data"]["vehicle"] == true){
				...
				if (event["data"]["showbelt"] == false){
					...
					if (event["data"]["hardness"] == 1){
						$("#hardBelt").css("color","#26ff38");
					} else {
						$("#hardBelt").css("color","#ff3737");
					}

					if (event["data"]["seatbelt"] == 1){
						$("#seatBelt").css("color","#26ff38");
					} else {
						$("#seatBelt").css("color","#ff3737");
					}
				}

				$("#gasoline").html("GAS <s>" + parseInt(event["data"]["fuel"]) + "</s>");
				$("#mph").html("KMH <s>" + parseInt(event["data"]["speed"] * 2.00934) + "</s>");
			} else {
...
//		$("#displayMiddle").html(event["data"]["radio"] + event["data"]["direction"] + "<s>:</s>" + event["data"]["street"] + "<s>:</s>" + event["data"]["hours"] + ":" + event["data"]["minutes"]);
