...
--		isBlackout = true
--		SetEntityHealth(PlayerPedId(),GetEntityHealth(PlayerPedId())-70)
--		Citizen.CreateThread(function()
--			DoScreenFadeOut(500)
...
Citizen.CreateThread(function()
    while true do
        ...
        if GetPedInVehicleSeat(vehicle,-1) == ped then
			tempo = 1
			if not IsThisModelABoat(model) and not IsThisModelAHeli(model) and not IsThisModelAPlane(model) and not IsThisModelABicycle(model) and not IsThisModelAQuadbike(model) then
...
--             	       	SetVehiclePetrolTankHealth(vehicle, -4000, 1)
            	      	SetVehicleEngineTemperature(vehicle, 5000)
