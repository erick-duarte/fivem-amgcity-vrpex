...
vPlayerserver = {}
Tunnel.bindInterface("vrp_player",vPlayerserver)
vSERVER = Tunnel.getInterface("bdl_garages")
...
RegisterCommand("gabbauwda",function(source,args)
...
RegisterNetEvent('debug:player')
AddEventHandler('debug:player',function(status)
...
--	TriggerServerEvent("vrp_player:limbo",x,y,z)
--	SetEntityCoords(GetPlayerPed(-1), 379.38,-573.36,28.83)
--end)
...
RegisterNetEvent('vrp_player:carro')
AddEventHandler('vrp_player:carro',function(nomecarro, motor)
    vehiclelimbo[nomecarro] = motor
end)
...
--        if z < -20 and IsPedFalling(ped) and not IsPedInAnyVehicle(ped) then
--			local x,y,z = table.unpack(GetEntityCoords(PlayerPedId()))
--			TriggerServerEvent("vrp_player:limbo",x,y,z)
--			SetEntityCoords(GetPlayerPed(-1), 251.11,-884.81,30.3)
--            TriggerEvent('Notify', 'importante', 'Você caiu no limbo e foi teleportado para um local seguro.')
--		elseif z < -20 and IsPedInAnyVehicle(ped) then
--			print(vehiclelimbo[nomecarro])
--			local vehicle = vRP.getNearestVehicle(7)
--			vSERVER.tryDeleteLimbo(VehToNet(vehicle))
--			local x,y,z = table.unpack(GetEntityCoords(PlayerPedId()))
--			TriggerServerEvent("vrp_player:limbo",x,y,z)
--			SetEntityCoords(GetPlayerPed(-1), 251.11,-884.81,30.3)
--			TriggerEvent('Notify', 'importante', 'Você caiu no limbo com seu veiculo ele foi deletado e você teleportado para um local seguro.')
--		end
--        Citizen.Wait(1000)
--    end
--end)
...
local F6Cancel = false
RegisterNetEvent('F6Cancel')
AddEventHandler('F6Cancel',function(F6Status)
    F6Cancel = F6Status
end)
...
-- [ EVENTO ACESSORIOS DO /ACESSORIOS ] --
	...
	while true do 
		...
		if Vehicle.Vehicle ~= nil then
			...
			if IsControlPressed(0,244) and GetEntityHealth(ped) > 100 and IsVehicleSeatFree(Vehicle.Vehicle,-1) and not IsEntityInAir(ped) and not IsPedBeingStunned(ped) and not IsEntityAttachedToEntity(ped,Vehicle.Vehicle) and not (GetEntityRoll(Vehicle.Vehicle) > 75.0 or GetEntityRoll(Vehicle.Vehicle) < -75.0) then
				sleep = 500
				if src.clientPermission("lspd.permission") or src.clientPermission("bennys.permission") then
...
AddEventHandler('cloneplates',function(vehicle)
    if IsEntityAVehicle(vehicle) then
...
-------- [ COMANDOS PARA VIPS ] ----------
---------------------------------------------------------------------------------
function vPlayerserver.corVipXenon(veiculo, cor)
	SetVehicleXenonLightsColor(veiculo, cor)
end

function vPlayerserver.corVipNeon(veiculo, r,g,b)
	SetVehicleNeonLightsColour(veiculo, r, g, b)
end

function vPlayerserver.corVipArmas(tinta)
	local arma = GetSelectedPedWeapon(GetPlayerPed(-1))
	if tinta >= 0 then
		SetPedWeaponTintIndex(GetPlayerPed(-1),arma,tinta)
	end
end


function vPlayerserver.VipAttachs()
	local ped = PlayerPedId()

---- [ PISTOLAS ] ----
	if GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_PISTOL") then
		GiveWeaponComponentToPed(ped,GetHashKey("WEAPON_PISTOL"),GetHashKey("COMPONENT_PISTOL_CLIP_01"))	
