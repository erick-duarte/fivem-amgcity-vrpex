...
Citizen.CreateThread(function()
	TriggerServerEvent("vrpdoorsystem:restartscript",source)
end)
...
Citizen.CreateThread(function()
	while true do
		...
		local x,y,z = table.unpack(GetEntityCoords(PlayerPedId()))
		for k,v in pairs(doors) do
			if GetDistanceBetweenCoords(x,y,z,v.x,v.y,v.z,true) <= 1.2 then
				tempo = 1
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(doors) do
			if GetDistanceBetweenCoords(x,y,z,v.x,v.y,v.z,true) <= 15 then
				door = GetClosestObjectOfType(v.x,v.y,v.z,1.0,v.hash,false,false,false)
				if door ~= 0 then
					SetEntityCanBeDamaged(door,false)
					if v.lock == false then
						--if v.text then
