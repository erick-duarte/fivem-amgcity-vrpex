local webhook_antiflood = "https://discord.com/api/webhooks/847476359730954250/vp67ti9W_UtcAYcQNz2KQwh4TOHmi_ggf7jrx_H0iRNY8Q8hsyrLNI2Sj_oBgcNw1mZG"
...
function vRP.antiflood(source,key,limite)
    ...
    if(flood[key][source]==nil)then
        ...
        if(os.time()-delayflood[key][source]<2)then
--			TriggerClientEvent("Notify",source,"importante","Para de ficar floodando, você será banido automaticamente.")
            flood[key][source]= flood[key][source] + 1
            if(flood[key][source]==limite)then
                local user_id = vRP.getUserId(source)
                vRP.setBanned(user_id,true)    
                DropPlayer(source, "Sistema de anti-flood. Você foi banido! Procure alguém da staff.")
