...
colorPickerP.on(["color:change"], function (color) {
    $.post('http://warm-bennys/changeItem',JSON.stringify({type: 'color', r: color.rgb.r, g: color.rgb.g, b: color.rgb.b}) )
});
// cor secundaria
...
colorPickerS.on([ "color:change"], function (color) {
    $.post('http://warm-bennys/changeItem',JSON.stringify({type: 'Scolor', r: color.rgb.r, g: color.rgb.g, b: color.rgb.b}) )
});
// cor smoke
...
colorPickerSmoke.on([ "color:change"], function (color) {
    $.post('http://warm-bennys/changeItem',JSON.stringify({type: 'smoke', r: color.rgb.r, g: color.rgb.g, b: color.rgb.b}) )
});
// cor neon
...
colorPickerNeon.on([ "color:change"], function (color) {
    $.post('http://warm-bennys/changeItem',JSON.stringify({type: 'neon', r: color.rgb.r, g: color.rgb.g, b: color.rgb.b}) )
});
...
function acceptMod() {
    $("#warning").hide();
    $.post('http://warm-bennys/changeCamera',JSON.stringify({ type: 'reset' }) )
    $('.menu-option').hide();
    $('#home').fadeIn();
    $('.sectionItem').show();
    $('#textContent').text('Mecânica');
    ...
    submenu = false;
    $.post('http://warm-bennys/acceptMod',JSON.stringify({price: price}));
}
...
function declinetMod() {
    ...
    $('#textContent').text('Mecânica');
    submenu = false;
    $.post('http://warm-bennys/declineMod' )
}
...
function selectHome(e) {   
    ...
    if (haveItem == 'true') {
        ...
        var id = $(e).attr("href")
        $.post('http://warm-bennys/changeCamera', JSON.stringify({ type: id }) )
...
function selectItem(element) {
    ...
    $.post('http://warm-bennys/changeItem',JSON.stringify({ type: element.dataset.type, value: element.dataset.value, part: element.dataset.part, partId: element.dataset.custom, before: beforeInstalled }) )
    submenu = true;
}

function selectColor(type, op, colorP){
    $.post('http://warm-bennys/changeItem',JSON.stringify({ type: 'customcolor', customColor: type, op: op, colorP: colorP }) )
}

function setXenonLights(op){
    $.post('http://warm-bennys/changeItem',JSON.stringify({ type: 'xenoncolor', op: op}) )
}
