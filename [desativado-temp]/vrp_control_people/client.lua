...
local listaCarros = {
    "italirsx", "as350prf", "corprf", "cruzeprf2", "golpmesp", "l200prf",
    "paliopmrp1", "spacerp", "spinpmesp", "sw4ft2019", "trailpf", "trailrota2", "trailprf", "trailrota2", "xtbaep",
    "trailpm1", "xre2019", "2015polstang", "hiluxpf", "rota4", "spinrp4", "sw4revrota1", "sw4pc1", "trail19goe", "trailbaep4", "trailcfp", "trailcivileie", "trailgarra1"
}

local listaPcs = {
--    {x=441.67236328125, y=-978.81140136719, z=30.689674377441, h=210.6392364502},
    {x=387.39, y=-674.49, z=29.28}
}
...
RegisterNUICallback("ButtonClick", function(data, cb)
    ...
    if data.action == "setMulta" then
        func.setRegistro(data.passaporte, "multa", data.descricao, data.img, data.valor)
        SendNUIMessage({type = 'reloadPassaporte'})
    end

    if data.action == "setOcorrencia" then
        func.setRegistro(data.passaporte, "ocorrencia", data.descricao,data.img, data.valor)
        SendNUIMessage({type = 'reloadPassaporte'})
    end

    if data.action == "setForagido" then
        func.setRegistro(data.passaporte, "foragido", data.descricao, data.img,data.valor)
        SendNUIMessage({type = 'reloadPassaporte'})
    end

    if data.action == "setPrisao" then
        func.setRegistro(data.passaporte, "prisao", data.descricao, data.pena, data.multa)
        SendNUIMessage({type = 'reloadPassaporte'})
    end
...
function funcClient.carroPrisao(id, tempo)
	...
	if HasModelLoaded(vhash) then
        ...
        pveh04 = CreatePedInsideVehicle(nveh,27,GetHashKey("s_m_y_swat_01"),3,true,false)
        pveh05 = CreatePedInsideVehicle(nveh,27,GetHashKey("s_m_y_swat_01"),4,true,false)
        pveh06 = CreatePedInsideVehicle(nveh,27,GetHashKey("s_m_y_swat_01"),6,true,false)
        pveh07 = CreatePedInsideVehicle(nveh,27,GetHashKey("s_m_y_swat_01"),7,true,false)
		...
		setPedPropertys(pveh05,armas[math.random(1,#armas)])
		setPedPropertys(pveh06,armas[math.random(1,#armas)])
        setPedPropertys(pveh07,armas[math.random(1,#armas)])
        ...
        SetEntityAsMissionEntity(pveh04,  true, false)
		SetEntityAsMissionEntity(pveh05,  true, false)
		SetEntityAsMissionEntity(pveh06,  true, false)
        SetEntityAsMissionEntity(pveh07,  true, false)
        ...
        async(function()
            ...
            while inTransport do
                ...
                if IsPedDeadOrDying(pveh01) and IsPedDeadOrDying(pveh02) and IsPedDeadOrDying(pveh03) and IsPedDeadOrDying(pveh04) and IsPedDeadOrDying(pveh05) and IsPedDeadOrDying(pveh06) and IsPedDeadOrDying(pveh07) then
                ...
                if distancia < 5.0 and math.ceil(vehspeed) < 3 then
                    if IsPedSittingInAnyVehicle(ped) then
                        ...
                        SetTimeout(3000, function()
                            ...
                            TriggerServerEvent("trydeleteped",PedToNet(pveh05))
                            TriggerServerEvent("trydeleteped",PedToNet(pveh06))
                            TriggerServerEvent("trydeleteped",PedToNet(pveh07))
                            ...
                            SetEntityAsNoLongerNeeded(pveh05)
                            SetEntityAsNoLongerNeeded(pveh06)
                            SetEntityAsNoLongerNeeded(pveh07)
