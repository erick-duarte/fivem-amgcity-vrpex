...
vRP._prepare("vRP/update_foto","UPDATE vrp_user_identities SET foto = @foto WHERE user_id = @user_id")
vRP._prepare("vRP/insert_policia","INSERT IGNORE INTO policia (police_id,user_id,dkey,dvalue,img,valor,datahora,id_pai) VALUES (@police_id, @user_id, @dkey, @dvalue, @img, @valor, NOW(),@id_pai)")
vRP._prepare("vRP/select","SELECT p.id, p.dvalue, p.img, p.valor, DATE_FORMAT(p.datahora, '%d/%m/%Y %H:%i') datahora, u.name, u.firstname FROM policia p INNER JOIN vrp_user_identities u ON u.user_id=p.police_id WHERE p.user_id = @user_id AND p.dkey like @dkey ORDER BY 1 DESC LIMIT 0,20")
vRP._prepare("vRP/select_all","SELECT p.id, p.dvalue, u2.foto, p.valor, DATE_FORMAT(p.datahora, '%d/%m/%Y %H:%i') datahora, u.name as nomePolicia,    u.firstname as sobrenomePolicia,    u2.name as nomeForagido,    u2.firstname as sobrenomeForagido FROM policia p INNER JOIN    vrp_user_identities u ON u.user_id = p.police_id        INNER JOIN    vrp_user_identities u2 ON u2.user_id = p.user_id WHERE p.dkey=@dkey ORDER BY 1 DESC LIMIT 0,20")

vRP._prepare("vRP/select_foragidos","SELECT p.id, p.user_id, p.dvalue, u2.foto, p.valor, DATE_FORMAT(p.datahora, '%d/%m/%Y %H:%i') datahora, u.name as nomePolicia,    u.firstname as sobrenomePolicia,    u2.name as nomeForagido,    u2.firstname as sobrenomeForagido FROM    policia p        INNER JOIN    vrp_user_identities u ON u.user_id = p.police_id        INNER JOIN    vrp_user_identities u2 ON u2.user_id = p.user_id WHERE    p.dkey = 'foragido' AND u2.foragido = 1 ORDER BY 1 DESC")

vRP._prepare("vRP/update_foragido","UPDATE vrp_user_identities SET foragido = @foragido WHERE user_id = @user_id")
vRP._prepare("vRP/delete_foragido","DELETE FROM policia WHERE dkey = 'foragido' AND user_id = @user_id")
...
function func.setRegistro(passaporte, dkey, dvalue, img, valor, id_pai)
    ...
    if dkey == "multa" then 
        func.setMulta(passaporte, valor) 
    end
    ...
    if dkey == "prisao" then 
        ...
        print(img)
        print("-----")
        func.setPrisao(passaporte, img) 
    end

    if dkey == "foragido" then
        vRP.execute("vRP/update_foragido",{foragido = valor, user_id = passaporte})
    end
