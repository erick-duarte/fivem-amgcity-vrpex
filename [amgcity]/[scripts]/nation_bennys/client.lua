...
function updateCart(myveh, modtype, index, colortype)
	...
	elseif modtype == "bulletproof" or modtype == "custom" or modtype == "fabrica" then
		if func.getTypeVeh(vname) == "vips" then
			if modtype == "fabrica" then
				if cart["bulletproof"] ~= nil then
					cart["total"] = cart["total"] - prices["bulletproof"].startpricevip
					cart["bulletproof"] = nil
				end
				if cart["custom"] ~= nil then
					cart["total"] = cart["total"] - prices["custom"].startpricevip
					cart["custom"] = nil
				end
		...
		elseif func.getTypeVeh(vname) == "import" then
			if modtype == "fabrica" then
				if cart["bulletproof"] ~= nil then
					cart["total"] = cart["total"] - prices["bulletproof"].startpriceimp
					cart["bulletproof"] = nil
				end
				if cart["custom"] ~= nil then
					cart["total"] = cart["total"] - prices["custom"].startpriceimp
					cart["custom"] = nil
				end
				SendNUIMessage({action = "price", price = cart["total"]})
				return
	...
	elseif modtype == "vidro" then
		if func.getTypeVeh(vname) == "vips" then
			if cart[modtype] == nil and myveh.windowtint ~= index and index > 0 then
				cart[modtype] = index
				cart["total"] = cart["total"] + (prices[modtype].startpricevip + prices[modtype].increaseby * p(index-1))
			elseif myveh.windowtint ~= index and index > 0 then
				cart["total"] = cart["total"] - (prices[modtype].startpricevip + prices[modtype].increaseby * p(cart[modtype]-1))
				cart[modtype] = index
				cart["total"] = cart["total"] + (prices[modtype].startpricevip + prices[modtype].increaseby * p(index-1))
		...
		elseif func.getTypeVeh(vname) == "import" then
			if cart[modtype] == nil and myveh.windowtint ~= index and index > 0 then
				cart[modtype] = index
				cart["total"] = cart["total"] + (prices[modtype].startpriceimp + prices[modtype].increaseby * p(index-1))
			elseif myveh.windowtint ~= index and index > 0 then
				cart["total"] = cart["total"] - (prices[modtype].startpriceimp + prices[modtype].increaseby * p(cart[modtype]-1))
				cart[modtype] = index
				cart["total"] = cart["total"] + (prices[modtype].startpriceimp + prices[modtype].increaseby * p(index-1))
			elseif (index < 1 or myveh.windowtint == index) and cart[modtype] ~= nil then
				cart["total"] = cart["total"] - (prices[modtype].startpriceimp + prices[modtype].increaseby * p(cart[modtype]-1))
	...
	elseif cart[tostring(modtype)] == nil and myveh.mods[modtype].mod ~= index and index > -1 then
		if func.getTypeVeh(vname) == "vips" then
			...
			cart["total"] = cart["total"] + (prices[modtype].startpricevip + prices[modtype].increaseby * index)
		elseif func.getTypeVeh(vname) == "import" then
			cart[tostring(modtype)] = index
			cart["total"] = cart["total"] + (prices[modtype].startpriceimp + prices[modtype].increaseby * index)
		else
			cart[tostring(modtype)] = index
	...
	elseif myveh.mods[modtype].mod ~= index and index > -1 then
		if func.getTypeVeh(vname) == "vips" then
			cart["total"] = cart["total"] - (prices[modtype].startpricevip + prices[modtype].increaseby * cart[tostring(modtype)])
			cart[tostring(modtype)] = index
			cart["total"] = cart["total"] + (prices[modtype].startpricevip + prices[modtype].increaseby * index)
		elseif func.getTypeVeh(vname) == "import" then
			cart["total"] = cart["total"] - (prices[modtype].startpriceimp + prices[modtype].increaseby * cart[tostring(modtype)])
			cart[tostring(modtype)] = index
			cart["total"] = cart["total"] + (prices[modtype].startpriceimp + prices[modtype].increaseby * index)
		else
